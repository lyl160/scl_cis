					...
					<div class="col-sm-12">
						<div>
							<label class="label_search">关键字:</label>
								<input id="keyword" type="text" name="keyword" placeholder="请输入关键字">
						...
						<div>
							<span>
								<a class="btn btn-small btn-add" onclick="dynamicAttrAdd();">新增</a>
								<a class="btn btn-small btn-update" onclick="dynamicAttrEdit();"> 修改 </a>
								<a class="btn btn-small btn-delete" onclick="dynamicAttrDelete();"> 删除</a>
							</span> 
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	jQuery(function($) {
		...
		jQuery(grid_selector).jqGrid({
			...
			colNames :['id','templateId','name','type','attrOption','seq','required','操作'],
			colModel : [
						{name : 'id',index : 'id',width : '20%' },
						{name : 'templateId',index : 'template_id',width : '20%' },
						{name : 'name',index : 'name',width : '20%' },
						{name : 'type',index : 'type',width : '20%' },
						{name : 'attrOption',index : 'attr_option',width : '20%' },
						{name : 'seq',index : 'seq',width : '20%' },
						{name : 'required',index : 'required',width : '20%' },
	...
	//新增
	function dynamicAttrAdd() {
		openDialog({
			dialogId : 'dlg-role',
			title : '新增巡检动态属性表',
			pageUrl : 'dynamicAttr/add/view',
			width : '500px',
		    height:'350px'
		});
	}
    ...
    function dynamicAttrDelete() {
        ...
        msg.confirm({title:'确认',position:'center',msg:'您确定要删除选中的数据吗？',call:function(ok){
            if(ok){
                $.ajax({
                    type : "post",
                    url : "dynamicAttr/deleteBatch?ids=" + ids,
                    dataType : 'json',

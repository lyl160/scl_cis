					...
					<div class="col-sm-12">
						<div>
							<label class="label_search">关键字:</label>
								<input id="keyword" type="text" name="name" placeholder="请输入全局设置名称">
						...
						<div>
							<span>
								<a class="btn btn-small btn-add" onclick="globalSettingsAdd();">新增</a>
								<a class="btn btn-small btn-update" onclick="globalSettingsEdit();"> 修改 </a>
								<!-- <a class="btn btn-small btn-delete" onclick="globalSettingsDelete();"> 删除</a> -->
							</span> 
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	//新增
	function globalSettingsAdd() {
		openDialog({
			dialogId : 'dlg-role',
			title : '新增全局设置',
			pageUrl : 'globalSettings/add/view',
			width : '500px',
		    height:'350px'
		});
	}
	//编辑
	function globalSettingsEdit() {
        ...
        openDialog({
            dialogId : 'dlg-role',
            title : '编辑全局设置',
            pageUrl :'globalSettings/edit/view?id='+rows,
            width :'500px',
            height:'350px',
    ...
    function globalSettingsDelete() {
        ...
        msg.confirm({title:'确认',position:'center',msg:'您确定要删除选中的数据吗？',call:function(ok){
            if(ok){
                $.ajax({
                    type : "post",
                    url : "globalSettings/deleteBatch?ids=" + ids,
                    dataType : 'json',

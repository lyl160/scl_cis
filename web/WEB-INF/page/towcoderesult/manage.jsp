...
<div class="row">
	<div class="col-xs-12">
		...
		<div style="margin-top: 10px">
			<span>
				<a class="btn btn-small btn-add" onclick="towCodeResultAdd();">新增</a>
				<a class="btn btn-small btn-update" onclick="towCodeResultEdit();"> 修改 </a>
				<a class="btn btn-small btn-delete" onclick="towCodeResultDelete();"> 删除</a>
			</span> 
...
<script type="text/javascript">
	...
	jQuery(function($) {
		...
		jQuery(grid_selector).jqGrid({
			...
			colNames :['id','createTime','twoText','twoId','操作'],
			colModel : [
						{name : 'id',index : 'id',width : '20%' },
						{name : 'createTime',index : 'create_time',width : '20%' },
						{name : 'twoText',index : 'two_text',width : '20%' },
						{name : 'twoId',index : 'two_id',width : '20%' },
	...
	//新增
	function towCodeResultAdd() {
		openDialog({
			dialogId : 'dlg-role',
			title : '新增会员积刈',
			pageUrl : 'towCodeResult/add/view',
			width : '500px',
		    height:'350px'
		});
	}
	//编辑
	function towCodeResultEdit() {
        ...
        openDialog({
            dialogId : 'dlg-role',
            title : '编辑会员积刈',
            pageUrl :'towCodeResult/edit/view?id='+rows,
            width :'500px',
            height:'350px',
    ...
    function towCodeResultDelete() {
        ...
        msg.confirm({title:'确认',position:'center',msg:'您确定要删除选中的数据吗？',call:function(ok){
            if(ok){
                $.ajax({
                    type : "post",
                    url : "towCodeResult/deleteBatch?ids=" + ids,
                    dataType : 'json',

...
function initPagingBar(gtable,$paging_bar){
	...
	pagebarLoad(gtable,page,$paging_bar);
}

//渲染
function pagebarLoad(gtable,page,$paging_bar){
	...
	if(pageTotal >=len ){
		...
		for (var i=page_1;i<=total;i++){
			...
			if(page == i){
				pageinfo  = pageinfo + "<li class=\"active\"><a href=\"javascript:pageIndex('"+gtable+"','"+i+"','"+$paging_bar+"')\">"+pageindex+"</a></li>";
			}else{
				pageinfo  = pageinfo + "<li><a href=\"javascript:pageIndex('"+gtable+"','"+i+"','"+$paging_bar+"')\">"+pageindex+"</a></li>";
			}
		}
		...
		{
			if(page == i){
				pageinfo  = pageinfo + "<li class=\"active\"><a href=\"javascript:pageIndex('"+gtable+"','"+i+"','"+$paging_bar+"')\">"+i+"</a></li>";
			}else{
				pageinfo  = pageinfo + "<li><a href=\"javascript:pageIndex('"+gtable+"','"+i+"','"+$paging_bar+"')\">"+i+"</a></li>";
			}
		}
	}
...
function pageIndex(gtable,index,paging_bar){
	...
	pagebarLoad(gtable,index,paging_bar);
...
function pageUp(gtable,paging_bar){
	...
	pagebarLoad(gtable,up,paging_bar);
	jQuery(gtable).jqGrid('setGridParam', {
        url : grid_url,
        page:up
    }).trigger("reloadGrid");
...
function pageNext(gtable,paging_bar){
	...
	pagebarLoad(gtable,next,paging_bar);
	jQuery(gtable).jqGrid('setGridParam', {
        url : grid_url,
        page:next
    }).trigger("reloadGrid");

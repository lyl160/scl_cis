...
var alertMsg = {
	...
	getBox : function() {
		...
		if ($box.length === 0) {
			$("body").append('<div id="rightFootMsgBox" class="rightFootMsgBox"><div class="tool" ><button type="button" class="close"><i class="ace-icon fa fa-times"></i></button></div></dvi>');
			$box = $('#rightFootMsgBox');
			$box.children(".tool").children(".close").on('click', alertMsg.closeAll);
		}
		return $box;
	},
	show : function(op) {
